		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'selectable':
					...
					$selected_options_sort_keys = explode(',', isset( $smof_data[ $value['id'] . '_selected_sorted'] ) ? $smof_data[ $value['id'] . '_selected_sorted'] : "" );
					foreach($all_options as $option_key => $option_value ) {

						if( isset( $smof_data[$value['id']][$option_key] ) && $smof_data[$value['id']][$option_key] == '1' ) {
					...
					foreach($selected_options_sort_keys as $key) {
						if( ! isset($selected_options[$key] ) ) {
							continue;
						}
						$selected_options_sorted[$key] = $selected_options[$key];
					}
					
					foreach($selected_options as $key => $_value) {

						if(!isset($selected_options_sorted[$key])) {
							$available_options[$key] = $_value;
						}
					}

					$selected_options = $selected_options_sorted;
					

					?>
					<div class="selectable-module">
						<ul class="options draggable droppable available">
						<?php

						foreach($available_options as $option_key => $option_value ) {
							echo '<li id="' . $value['id'] . '-' . $option_key . '">' . $option_value . '<input type="hidden" name="' . $value['id'] . '[' . $option_key . ']" data-key="' . $option_key . '" value="0"></li>';
						}
						?>

						</ul>
						<ul class="options droppable sortable selected">
							<?php

							foreach($selected_options as $option_key => $option_value ) {
								echo '<li id="' . $value['id'] . '-' . $option_key . '">' . $option_value . '<input type="hidden" name="' . $value['id'] . '[' . $option_key . ']" data-key="' . $option_key . '" value="1"></li>';
							}
							?>
						</ul>
						<input type="hidden" value="<?php echo isset( $smof_data[$value['id'] . '_selected_sorted'] ) ? $smof_data[$value['id'] . '_selected_sorted'] : '' ?>" class="selected-sorted" name="<?php echo $value['id'] ?>_selected_sorted">
					</div>
					...
					(function($) {
						...
						function sort(event, ui) {
							var keys = [];
							var item = ui.item ? ui.item : ui.draggable;
							var parent = $(item).parents('.selectable-module').eq(0);
							$(parent).find('.options.selected input').map(function() {
								keys.push( $(this).data('key') );
							});
							$(parent).find('.selected-sorted').val(keys.join(','));
						}
						...
						$(function() {
							$('.droppable').droppable({
								drop : function(event, ui) {
									...
									var isDraggable = ui.draggable.hasClass('ui-draggable');
									...
									var element = ui.draggable.clone().attr('class', '').attr('style', '');
									
									if( $(this).find('#' + ui.draggable.attr('id')).length == 0 ) {
										element.find('input').val(isSelected ? 1 : 0);
										$(this).append(element);
									}
				...
				case 'nested_pages':
					...
					$theValue = ($smof_data[$value['id']]);
					if(!is_array($theValue)) {
						$theValue = array();
					}

					$output .= '<select class="of-select" name="' . $value['id'] . '[]" id="' . $value['id'] . '" multiple="multiple" style="height: 200px">';

					foreach($pages as $_page) {
						$output .= '<option value="' . $_page->ID . '"' . ( in_array($_page->ID, $theValue) ? ' selected="selected"' : '' )  . '>' . ( $_page->post_parent != 0 ? '-- ' : ' ' ) . $_page->post_title . '</option>';
					}
				...
				case 'reset_colors':
					...
					ob_start();
					?>
					<button class="button-primary" id="reset-colors"><?php echo __('Reset skin colors') ?></button>
					...
					(function($) {
						$(function() {
							$('#reset-colors').on('click', function() {
								...
								var buttons = $('.wp-picker-input-wrap input.wp-picker-default');
